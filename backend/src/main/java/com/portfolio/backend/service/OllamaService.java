...
public class OllamaService {
  ...
  public PortfolioData parseResume(String resumeText) {
    String prompt = String.format(
        """
            You are an expert resume parser. Extract data from the provided resume and return ONLY valid JSON matching this structure (no markdown, no extra text). Do not fabricate values. Keep personal names exactly as written in the resume and NEVER replace a name with an address, company, or location:

            {
              "fullName": "string (person's name from resume — no addresses or locations)",
